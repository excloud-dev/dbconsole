...
import { displayTableName, quoteIdent } from "@/lib/sql/quote-ident"
...
export function DbConsole() {
  ...
  const handleViewTable = (tableName: string) => {
    ...
    if (currentTab && currentQueryEmpty) {
      setTabs((prev) =>
        prev.map((t) =>
          t.id === currentTab.id ? { ...t, query, pagination, name: `${displayTableName(tableName)} (top 100)` } : t,
        ),
      )
      if (activeConnection) {
    ...
    const newTab = { id: newId, name: `${displayTableName(tableName)} (top 100)`, query, pagination, connectionId: activeConnection ?? undefined }
    setTabs([...tabs, newTab])
    setActiveTab(newId)
